...
use crate::{BundleFormat, Cli, Commands, Verbosity, serve::ServeUpdate};
        ...
        let filter = if env::var(LOG_ENV).is_ok() {
            ...
            EnvFilter::new(
                "error,dx=trace,dioxus_cli=trace,manganis_cli_support=trace,wasm_split_cli=trace,\
                 subsecond_cli_support=trace",
            )
        } else {
            EnvFilter::new(format!(
                "error,dx={our_level},dioxus_cli={our_level},manganis_cli_support={our_level},\
                 wasm_split_cli={our_level},subsecond_cli_support={our_level}",
