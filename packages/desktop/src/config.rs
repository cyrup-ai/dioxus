...
use wry::{
    RequestAsyncResponder, WebViewId,
    http::{Request as HttpRequest, Response as HttpResponse},
};

use crate::{
    ipc::UserWindowEvent,
    menubar::{DioxusMenu, default_menu_bar},
};
...
impl Config {
    ...
    where F: Fn(WebViewId, HttpRequest<Vec<u8>>) -> HttpResponse<Cow<'static, [u8]>> + 'static {
    ...
    /// #
    /// # fn main() {
    /// let cfg = Config::new().with_asynchronous_custom_protocol(
    ///     "asset",
    ...
    where F: Fn(WebViewId, HttpRequest<Vec<u8>>, RequestAsyncResponder) + 'static {
