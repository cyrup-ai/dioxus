...
impl AppServer {
    ...
    pub(crate) async fn open(
        ...
        if should_open {
            ...
            if self.client.builds_opened == 0 {
                tracing::info!(
                    "Build completed successfully in {GLOW_STYLE}{:?}ms{GLOW_STYLE:#}, launching \
                     app! 💫",
    ...
    pub(crate) async fn open_debugger(&mut self, dev: &WebServer, build: BuildId) {
        if self.use_hotpatch_engine {
            tracing::warn!(
                "Debugging symbols might not work properly with hotpatching enabled. Consider \
                 disabling hotpatching for debugging."
            );
        }
...
fn create_notify_watcher(
    ...
    const NOTIFY_ERROR_MSG: &str = "Failed to create file watcher.\nEnsure you have the required \
                                    permissions to watch the specified directories.";
