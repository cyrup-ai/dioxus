...
use quote::quote;
use syn::{
    GenericArgument, PathArguments, PathSegment, Type, parse::Error, parse_quote,
...
mod field_info {
    ...
    use syn::{Expr, Path, parse::Error, parse_quote, punctuated::Punctuated, spanned::Spanned};
...
mod struct_info {
    ...
    use syn::{Expr, Ident, parse::Error, parse_quote, punctuated::Punctuated, spanned::Spanned};

    use super::{
        child_owned_type,
        field_info::{FieldBuilderAttr, FieldInfo},
        looks_like_callback_type, looks_like_signal_type,
        util::{
...
Finally, call `.build()` to create the instance of `{name}`.
        ...
        pub fn field_impl(&self, field: &FieldInfo) -> Result<TokenStream, Error> {
            ...
            if *field_name == "key" {
                return Err(Error::new_spanned(
                    field_name,
                    "Naming a prop `key` is not allowed because the name can conflict with the built in key attribute. See https://dioxuslabs.com/learn/0.6/reference/dynamic_rendering#rendering-lists for more information about keys",
                ));
            }
            let StructInfo {
                ref builder_name, ..
