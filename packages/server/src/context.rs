...
mod server_fn_impl {
    ...
    impl DioxusServerContext {
        ...
        fn response_parts_mut(&self) -> RwLockWriteGuard<'_, http::response::Parts> {
            ...
            {
                tracing::error!(
                    "Attempted to modify the request after the first frame of the response has \
                     already been sent. You can read the response, but modifying the response \
                     will not change the response that the client has already received. Try \
                     modifying the response before the suspense boundary above the router is \
                     resolved."
                );
            }
