...
use dioxus_core::{
    DynamicNode, ErrorContext, ScopeId, SuspenseContext, VNode, VirtualDom, has_context,
    provide_error_boundary,
};
use dioxus_fullstack_hooks::{StreamingContext, StreamingStatus, history::FullstackHistory};
...
use crate::{
    DioxusServerContext, ProvideServerContext, ServeConfig, StreamingMode,
...
impl SsrRendererPool {
    ...
    async fn render_to<T: FnOnce() -> VirtualDom + Send + Sync + 'static>(
        self: Arc<Self>,
        cfg: ServeConfig,
        route: String,
        virtual_dom_factory: T,
        server_context: DioxusServerContext,
    ) -> Result<
        (
            RenderFreshness,
            impl Stream<Item = Result<String, dioxus_isrg::IncrementalRendererError>> + use<T>,
        ),
        SSRError,
    > {
        ...
        let join_handle = spawn_platform(move || {
            ProvideServerContext::new(create_render_future(), server_context_clone)
        });
...
impl SSRState {
    ...
    pub async fn render<T: FnOnce() -> VirtualDom + Send + Sync + 'static>(
        &self,
        route: String,
        cfg: ServeConfig,
        virtual_dom_factory: T,
