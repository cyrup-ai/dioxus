...
use dioxus_core::{HasAttributes, IntoAttributeValue};
...
macro_rules! mod_method_mapping {
    ($matching:ident; $(#[$attr:meta])* $name:ident;) => {
        if $matching == stringify!($name) {
            return Some((stringify!($name), None));
        }
    };
    ($matching:ident; $(#[$attr:meta])* $name:ident : $lit:literal;) => {
        if $matching == stringify!($name) {
            return Some(($lit, None));
        }
    };
    ($matching:ident; $(#[$attr:meta])* $name:ident : $lit:literal in $ns:literal;) => {
        if $matching == stringify!($name) {
            return Some(($lit, Some($ns)));
        }
    };
    ($matching:ident; $(#[$attr:meta])* $name:ident in $ns:literal;) => {
        if $matching == stringify!($name) {
