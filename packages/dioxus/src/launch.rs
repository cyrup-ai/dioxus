...
impl LaunchBuilder {
    ...
    #[cfg_attr(
        ...
        deprecated(
            note = "No renderer is enabled. You must enable a renderer feature on the dioxus \
                    crate before calling the launch function.\nAdd `web`, `desktop`, `mobile`, or \
                    `fullstack` to the `features` of dioxus field in your Cargo.toml.\n# \
        ...
        let platform = if cfg!(feature = "native") {
            ...
            panic!(
                "No platform feature enabled. Please enable one of the following features: \
                 liveview, desktop, mobile, web, tui, fullstack to use the launch API."
            )
        };
    ...
    pub fn launch(self, app: fn() -> Element) {
        ...
        if cfg!(feature = "third-party-renderer") {
            panic!(
                "No first party renderer feature enabled. It looks like you are trying to use a \
                 third party renderer. You will need to use the launch function from the third \
                 party renderer crate."
            );
        }

        panic!(
            "No platform feature enabled. Please enable one of the following features: liveview, \
             desktop, mobile, web, tui, fullstack to use the launch API."
        )
    }
}
