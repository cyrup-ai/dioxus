...
impl ServeConfigBuilder {
    ...
    ///         dioxus_server::ServeConfigBuilder::default()
...
impl std::fmt::Display for UnableToLoadIndex {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Failed to find index.html. Make sure the index_path is set correctly and the WASM \
             application has been built. Tried to open file at path: {:?}",
            self.0
        )
    }
}
...
fn load_index_html(contents: String, root_id: &'static str) -> IndexHtml {
    let (pre_main, post_main) = contents
        ...
        .unwrap_or_else(|| {
            panic!(
                "Failed to find id=\"{root_id}\" in index.html. The id is used to inject the \
