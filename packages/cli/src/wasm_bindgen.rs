...
impl WasmBindgen {
    ...
    async fn install(&self) -> anyhow::Result<()> {
        ...
        if let Err(e) = self.install_binstall().await {
            tracing::error!("Failed to install wasm-bindgen-cli@{}: {e}", self.version);
            tracing::info!(
                "Failed to install prebuilt binary for wasm-bindgen-cli@{}. Compiling from source \
                 instead. This may take a while.",
                self.version
            );
        } else {
            tracing::info!(
    ...
    async fn verify_local_install(&self) -> anyhow::Result<()> {
        ...
        if installed_version != self.version {
            return Err(anyhow!(
                "Incorrect wasm-bindgen-cli version: project requires version {} but version {} \
                 is installed",
                self.version,
                installed_version,
            ));
