...
impl DynamicLiteralPool {
    ...
    pub fn component_property<T: 'static>(
        ...
        let grab_float = || {
            self.get_component_property_or_default(id, hot_reload, HotReloadLiteral::as_float)
                .unwrap_or_else(|| {
                    tracing::error!(
                        "Expected a float component property, because the type was {}. The CLI \
                         gave the hot reloading engine a type of {:?}. This is probably caused by \
                         a bug in dioxus hot reloading. Please report this issue.",
                        std::any::type_name::<T>(),
                        hot_reload.component_values.get(id)
                    );
                    Default::default()
                })
        };
        let grab_int = || {
            self.get_component_property_or_default(id, hot_reload, HotReloadLiteral::as_int)
                .unwrap_or_else(|| {
                    tracing::error!(
                        "Expected a integer component property, because the type was {}. The CLI \
                         gave the hot reloading engine a type of {:?}. This is probably caused by \
                         a bug in dioxus hot reloading. Please report this issue.",
                        std::any::type_name::<T>(),
                        hot_reload.component_values.get(id)
        ...
        let grab_bool = || {
            self.get_component_property_or_default(id, hot_reload, HotReloadLiteral::as_bool)
                .unwrap_or_else(|| {
                    tracing::error!(
                        "Expected a bool component property, because the type was {}. The CLI \
                         gave the hot reloading engine a type of {:?}. This is probably caused by \
                         a bug in dioxus hot reloading. Please report this issue.",
                        std::any::type_name::<T>(),
                        hot_reload.component_values.get(id)
        ...
        let grab_fmted = || {
            self.get_component_property_or_default(id, hot_reload, |fmted| {
                HotReloadLiteral::as_fmted(fmted).map(|segments| self.render_formatted(segments))
            })
            .unwrap_or_else(|| {
                tracing::error!(
                    "Expected a string component property, because the type was {}. The CLI gave \
                     the hot reloading engine a type of {:?}. This is probably caused by a bug in \
                     dioxus hot reloading. Please report this issue.",
                    std::any::type_name::<T>(),
                    hot_reload.component_values.get(id)
                );
                Default::default()
