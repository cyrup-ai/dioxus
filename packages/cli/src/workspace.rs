...
use krates::{Cmd, KrateDetails, Krates, LockOptions, NodeId, semver::Version};
use target_lexicon::Triple;
use tokio::process::Command;

use crate::{AndroidTools, CliSettings, Result, config::DioxusConfig, styles::GLOW_STYLE};
...
impl Workspace {
    ...
    pub async fn current() -> Result<Arc<Workspace>> {
        ...
        let spin_future = async move {
            ...
            for x in 1..=100 {
                ...
                if x % 10 == 0 {
                    println!(
                        "{GLOW_STYLE}warning{GLOW_STYLE:#}: maybe check your network connection \
                         or build lock?"
                    );
                }
            }
    ...
    pub fn android_tools(&self) -> Result<Arc<AndroidTools>> {
        self.android_tools.clone().context(
            "Android not installed properly. Please set the `ANDROID_NDK_HOME` environment \
             variable to the root of your NDK installation.",
        )
    }
    ...
    pub fn find_main_package(&self, package: Option<String>) -> Result<NodeId> {
        if let Some(package) = package {
            ...
            if found.is_none() {
                tracing::error!(
                    "Could not find package {package} in the workspace. Did you forget to add it \
                     to the workspace?"
                );
        ...
        let kid = closest_parent.map(|(id, _)| id).with_context(|| {
            let bin_targets = self
                .krates
                .workspace_members()
                ...
                .collect::<Vec<_>>();
            format!(
                "Failed to find binary package to build.\nYou need to either run dx from inside a \
                 binary crate or specify a binary package to build with the `--package` flag. Try \
                 building again with one of the binary packages in the workspace:\n{}",
                bin_targets.join("\n")
            )
        })?;
