    ...
    pub fn log(&self, component: &str) {
        match self {
            ...
            ExtractSingleTextNodeError::NonTextNode => {
                tracing::error!(
                    "Error while rendering {component}: The children of {component} must be a \
                     single text node"
                );
            }
    ...
    for additional_attribute in additional_attributes {
        let attribute_value_as_string = match &additional_attribute.value {
            ...
            dioxus_core::AttributeValue::Listener(_) | dioxus_core::AttributeValue::Any(_) => {
                tracing::error!(
                    "document::* elements do not support event listeners or any value attributes. \
                     Expected displayable attribute, found {:?}",
