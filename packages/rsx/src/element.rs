...
impl Element {
    pub(crate) fn add_merging_non_string_diagnostic(diagnostics: &mut Diagnostics, span: Span) {
        diagnostics.push(span.error("Cannot merge non-fmt literals").help(
            "Only formatted strings can be merged together. If you want to merge literals, you \
...
mod tests {
    ...
    fn merge_conditional_attributes() {
        ...
        assert_eq!(
            attr.to_token_stream().pretty_unparse().as_str(),
            "::std::format!(\n    \"foo {0:} {1:}\",\n    { if true { \"bar\".to_string() } else \
             { ::std::string::String::new() } },\n    { if false { \"baz\".to_string() } else { \
             \"qux\".to_string() } },\n)"
        );
    ...
    fn merge_all_attributes() {
        ...
        if cfg!(debug_assertions) {
            assert_eq!(
                attr.to_token_stream().pretty_unparse().as_str(),
                "::std::format!(\n    \"foo {0:} {1:} {2:}\",\n    bar,\n    { if true { \
                 \"baz\".to_string() } else { ::std::string::String::new() } },\n    { if false { \
                 ::std::format!(\"{qux}\").to_string() } else { \"quux\".to_string() } },\n)"
            );
        } else {
            assert_eq!(
                attr.to_token_stream().pretty_unparse().as_str(),
                "::std::format!(\n    \"foo {0:} {1:} {2:}\",\n    bar,\n    { if true { \
                 \"baz\".to_string() } else { ::std::string::String::new() } },\n    { if false { \
                 (qux).to_string().to_string() } else { \"quux\".to_string() } },\n)"
            );
        }
