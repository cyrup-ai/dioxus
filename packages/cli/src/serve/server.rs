...
impl WebServer {
    ...
    pub(crate) fn start(runner: &AppServer) -> Result<Self> {
        ...
        let listener = std::net::TcpListener::bind(devserver_bind_address).with_context(|| {
            anyhow::anyhow!(
                "Failed to bind server to: {devserver_bind_address}, is there another devserver \
                 running?\nTo run multiple devservers, use the --port flag to specify a different \
                 port"
            )
        })?;
            ...
            |error| {
                Response::builder()
                    ...
                    .body(Body::from(format!(
                        "Backend connection failed. The backend is likely still starting up. \
                         Please try again in a few seconds. Error: {error:#?}"
                    )))
                    .unwrap()
            },
        ));
