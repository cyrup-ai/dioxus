...
pub(crate) async fn connectivity_check() -> Result<()> {
    ...
    for x in 0..=5 {
        ...
        if x == 0 {
            ...
            println!(
                "{GLOW_STYLE}warning{GLOW_STYLE:#}: ({x}/5) Taking a while, maybe your internet \
                 is down?"
            );
        }
    }
    ...
    bail!(
        "Error connecting to template repository. Try cloning the template manually or add \
         `dioxus` to a `cargo new` project."
    )
}
