...
impl HTMLData {
    ...
    fn get<T: serde::de::DeserializeOwned>(&self, index: usize) -> Result<T, TakeDataError> {
        if index >= self.data.len() {
            tracing::trace!(
                "Tried to take more data than was available, len: {}, index: {}; This is normal \
                 if the server function was started on the client, but may indicate a bug if the \
                 server function result should be deserialized from the server",
                self.data.len(),
                index
            );
            return Err(TakeDataError::DataNotAvailable);
                ...
                Err(err) => {
                    #[cfg(debug_assertions)]
                    {
                            ...
                            (debug_type, debug_locations)
                        {
                            ...
                            tracing::error!(
                                "Error deserializing data: {err:?}\n\nThis type was serialized on \
                                 the server at {debug_locations} with the type name {debug_type}. \
                                 The client failed to deserialize the type {client_type} at \
                                 {client_location}.",
                            );
