...
use crate::{
    ...
    innerlude::{CapturedError, SchedulerMsg, SuspenseContext, throw_into},
...
impl Scope {
    ...
    pub fn use_hook<State: Clone + 'static>(&self, initializer: impl FnOnce() -> State) -> State {
        ...
        let mut hooks = self.hooks.try_borrow_mut().expect(
            "The hook list is already borrowed: This error is likely caused by trying to use  \
             hook inside a hook which violates the rules of hooks.",
        );
