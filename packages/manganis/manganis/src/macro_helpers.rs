...
#[deprecated(note = "Relative asset!() paths are not supported. Use a path like \
                     `/assets/myfile.png` instead of `./assets/myfile.png`")]
...
pub const fn deserialize_asset(bytes: &[u8]) -> BundledAsset {
    ...
    match const_serialize::deserialize_const!(BundledAsset, bytes.read()) {
        Some((_, asset)) => asset,
        None => panic!(
            "Failed to deserialize asset. This may be caused by a mismatch between your dioxus \
             and dioxus-cli versions"
        ),
    }
}
