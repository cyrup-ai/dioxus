...
use crate::{Error, Result, TraceSrc, config::WebProxyConfig};
    ...
    any(move |parts: Parts, mut req: Request<MyBody>| async move {
        ...
        if req.headers().get("x-proxied-by-dioxus").is_some() {
            return Err(Response::builder()
                ...
                .body(Body::from(
                    "API is sharing a loopback with the dev server. Try setting a different port \
                     on the API config.",
                ))
                .unwrap());
        }
