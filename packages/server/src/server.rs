...
use tower::{ServiceExt, util::MapResponse};
use tower_http::services::fs::ServeFileSystemResponseBody;

use crate::{
    ContextProviders, DioxusServerContext, ProvideServerContext, SSRState, ServeConfig,
    render::SSRError, with_server_context,
};
...
pub trait DioxusRouterFnExt<S> {
    ...
    ///     let router = axum::Router::new()
    ///         // Register server functions routes with the default handler
    ///         .register_server_functions_with_context(Arc::new(vec![Box::new(|| {
    ///             Box::new(1234567890u32)
    ///         })]))
    ///         .into_make_service();
    ///     let listener = tokio::net::TcpListener::bind(addr).await.unwrap();
    ///     axum::serve(listener, router).await.unwrap();
...
/// use dioxus::prelude::*;
/// use dioxus_server::{RenderHandleState, ServeConfig, render_handler};
///
/// fn app() -> Element {
///     rsx! {
///         "hello!"
...
pub async fn render_handler(
    ...
    // Extract all necessary data before any async operations
    ...
    // Create a new server context with info from the request
    let parts = Arc::new(parking_lot::RwLock::new(parts));

    // Clone all necessary values before the async block
    let ssr_state = state.ssr_state().clone();
    let config = state.config.clone();
    let build_virtual_dom_closure = state.build_virtual_dom.clone();
    let context_providers = config.context_providers.clone();

    // Create the server context and add providers synchronously
    let server_context = DioxusServerContext::from_shared_parts(parts);

    // Add context providers directly to the server_context
    for provider in context_providers.as_slice() {
        let value = provider();
        // Convert to Box<dyn Any + Send + Sync + 'static>
        let value = Box::new(
            *Box::<dyn Any>::downcast::<Box<dyn Any + Send + Sync + 'static>>(value).unwrap(),
        );
        server_context.insert_any(value);
    }

    // Create the build_virtual_dom function that doesn't capture any references
    let build_virtual_dom = move || {
        let mut vdom = build_virtual_dom_closure();
    ...
    // Now execute the async render operation
    // Clone server_context for use after render since render takes ownership
    let server_context_for_response = server_context.clone();
    match ssr_state
        .render(url, config, build_virtual_dom, server_context)
        .await
    {
        Ok((freshness, rx)) => {
            let mut response = axum::response::Html::from(Body::from_stream(rx)).into_response();
            freshness.write(response.headers_mut());
            server_context_for_response.send_response(&mut response);
            response
        }
        Err(SSRError::Incremental(e)) => {
            // Convert incremental error to a 500 response
            report_err(format!("Server error: {}", e))
        }
        Err(SSRError::Routing(e)) => {
            // Convert routing error to a 404 response
            let status = StatusCode::NOT_FOUND;
            let body = Body::from(format!("Page not found: {}", e));
