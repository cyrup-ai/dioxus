...
mod js {
    ...
    fn replace_with(id: u32, n: u16) {
        "{const root = this.nodes[$id$]; let els = this.stack.splice(this.stack.length-$n$); if \
         (root.listening) { this.removeAllNonBubblingListeners(root); } root.replaceWith(...els);}"
    }
    ...
    fn remove(id: u32) {
        "{let node = this.nodes[$id$]; if (node !== undefined) { if (node.listening) { \
         this.removeAllNonBubblingListeners(node); } node.remove(); }}"
    }
    fn create_raw_text(text: &str) {
        "{this.stack.push(document.createTextNode($text$));}"
    }
    fn create_text_node(text: &str, id: u32) {
        "{let node = document.createTextNode($text$); this.nodes[$id$] = node; \
    ...
    fn remove_event_listener(event_name: &str<u8, evt>, id: u32, bubbles: u8) {
        "{let node = this.nodes[$id$]; node.listening -= 1; \
         node.removeAttribute('data-dioxus-id'); this.removeListener(node, $event_name$, \
         $bubbles$);}"
    }
    ...
    fn replace_placeholder(ptr: u32, len: u8, n: u16) {
        "{let els = this.stack.splice(this.stack.length - $n$); let node = this.loadChild($ptr$, \
         $len$); node.replaceWith(...els);}"
    }
    fn load_template(tmpl_id: u16, index: u16, id: u32) {
        "{let node = this.templates[$tmpl_id$][$index$].cloneNode(true); this.nodes[$id$] = node; \
         this.stack.push(node);}"
    }
    ...
    fn replace_placeholder_ref(array: &[u8], n: u16) {
        "{let els = this.stack.splice(this.stack.length - $n$); let node = \
         this.loadChild($array$); node.replaceWith(...els);}"
    }
}
